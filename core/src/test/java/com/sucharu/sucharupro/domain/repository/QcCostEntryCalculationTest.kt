...
class QcCostEntryCalculationTest {
    ...
    @Test
    fun `total cost is calculated accurately as quantity times unit cost`() = runBlocking {
        val result = repository.createCostEntry(
            projectId = "PRJ-01",
            productionJobId = "JOB-01",
            costType = QcCostType.REWORK_QC,
            description = "Rework inspection labor",
            quantity = 3.5,
            unitCost = 200.0,
            recordedBy = "insp-01",
        ...
        assertEquals(700.0, entry.totalCost, 0.001)
    }
}
