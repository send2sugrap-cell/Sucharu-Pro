...
class VendorDebitNoteDuplicateTest {
    ...
    @Test
    fun `two distinct debit note adjustments generate distinct debit note numbers`() = runBlocking {
        ...
        val post1 = (adjustmentRepository.postAdjustment(adj1.adjustmentId, "PURCHASE_RETURN", "acct-2", UserRole.ACCOUNTS) as DomainResult.Success).data
        ...
        val post2 = (adjustmentRepository.postAdjustment(adj2.adjustmentId, "PURCHASE_RETURN", "acct-2", UserRole.ACCOUNTS) as DomainResult.Success).data

        val dn1 = (adjustmentRepository.getDebitNoteById(post1.debitNoteId!!, UserRole.ACCOUNTS) as DomainResult.Success).data
        val dn2 = (adjustmentRepository.getDebitNoteById(post2.debitNoteId!!, UserRole.ACCOUNTS) as DomainResult.Success).data

        assertNotEquals(dn1.debitNoteId, dn2.debitNoteId)
        assertNotEquals(dn1.debitNoteNo, dn2.debitNoteNo)
    }
}
