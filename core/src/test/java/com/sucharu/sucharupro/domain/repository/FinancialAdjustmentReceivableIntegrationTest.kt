...
class FinancialAdjustmentReceivableIntegrationTest {
    ...
    @Test
    fun `posting credit adjustment attached to receivable settles and reduces receivable balance`() = runBlocking {
        val receivableRes = customerReceivableRepository.createReceivable(
            projectId = "PRJ-01",
            customerId = "CUST-001",
            ...
            description = "Receivable for invoice #INV-1001",
            actorId = "acct-1",
            callerRole = UserRole.ACCOUNTS
        )
        val receivableId = (receivableRes as DomainResult.Success).data.receivableId
        ...
        val adj = (adjustmentRepository.createAdjustment(
            ...
            description = "Credit note for partial damage",
        ...
        val updatedReceivable = (customerReceivableRepository.getReceivableById(receivableId, UserRole.ACCOUNTS) as DomainResult.Success).data
        assertEquals(Money(BigDecimal("3000.00")), updatedReceivable.settledAmount)
        assertEquals(Money(BigDecimal("7000.00")), updatedReceivable.outstandingAmount)
        assertEquals(CustomerReceivableStatus.PARTIALLY_SETTLED, updatedReceivable.status)
    }
}
