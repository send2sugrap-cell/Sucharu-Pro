...
/**
 * Category H: Idempotency Tests for Production Handoff (Module 05 Step 05).
 */
class ProductionHandoffIdempotencyTest {
    ...
    @Test
    fun repeatedHandoffCalls_areIdempotentAndReturnSameRecord() = runBlocking {
        val call1 = handoffRepository.authorizeProductionHandoff(
            approvalId = "appr-idemp-01",
            authorizedBy = "mgr-01",
            timestamp = "2026-08-16T11:00:00Z",
            callerRole = UserRole.MANAGER
        )
        assertTrue(call1 is DomainResult.Success)
        val handoff1 = (call1 as DomainResult.Success).data

        val call2 = handoffRepository.authorizeProductionHandoff(
            approvalId = "appr-idemp-01",
            authorizedBy = "mgr-01",
            timestamp = "2026-08-16T11:05:00Z",
            callerRole = UserRole.MANAGER
        )
        assertTrue(call2 is DomainResult.Success)
        val handoff2 = (call2 as DomainResult.Success).data

        assertEquals(handoff1.handoffId, handoff2.handoffId)

        val count = handoffRepository.observeHandoffs().first().size
