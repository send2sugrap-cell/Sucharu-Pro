...
class DeliveryAnalyticsConcurrencyTest {
    ...
    @Test
    fun `concurrent reads and alert resolutions execute safely`() = runBlocking {
        ...
        val deferredAcknowledge = async {
            repository.acknowledgeAlert("ALT-1", "user-1", UserRole.ADMIN)
        }

        val readResults = deferredReads.awaitAll()
        val ackResult = deferredAcknowledge.await()

        readResults.forEach { assertTrue(it is DomainResult.Success) }
        assertTrue(ackResult is DomainResult.Success)
    }
}
