...
/**
 * End-to-end HTTP Security Edge Test for Module 17 Step 03 Commercial Commitment & Conversion API.
 * Verifies authentication, RBAC authorization, and API route security.
 */
class CommercialCommitmentSecurityEdgeTest {
    ...
    @Test
    fun `unauthenticated request to convert quote to order returns 401`() {
        val (status, _) = makeRequest(
            path = "/api/v1/printing-quotes/QUO-001/convert-to-order",
            method = "POST",
            bodyJson = """{"priority":"NORMAL"}"""
        )
        assertEquals(401, status)
    }

    @Test
    fun `unauthorized role request to convert quote returns 403`() {
    ...
    @Test
    fun `unauthenticated request to get commitment returns 401`() {
        val (status, _) = makeRequest(
            path = "/api/v1/commercial-commitments/COMM-001",
            method = "GET"
        )
        assertEquals(401, status)
    }
