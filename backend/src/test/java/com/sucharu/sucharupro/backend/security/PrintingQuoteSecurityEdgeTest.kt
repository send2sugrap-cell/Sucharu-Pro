...
/**
 * End-to-end HTTP Security Edge Test for Module 17 Step 02 Printing Quotation Engine API.
 * Verifies authentication, RBAC authorization, and API route security.
 */
class PrintingQuoteSecurityEdgeTest {
    ...
    @Test
    fun `unauthenticated request to create quote returns 401`() {
        val (status, body) = makeRequest(
            path = "/api/v1/printing-quotes",
            method = "POST",
            bodyJson = """{"calculationId":"CALC-001","jobTitle":"Brochure"}"""
        )
        assertEquals(401, status)
    }

    @Test
    fun `unauthorized role request to create quote returns 403`() {
    ...
    @Test
    fun `unauthenticated request to get quote returns 401`() {
        val (status, _) = makeRequest(
            path = "/api/v1/printing-quotes/QUO-TEST-001",
            method = "GET"
        )
        assertEquals(401, status)
    }
