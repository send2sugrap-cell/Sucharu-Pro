...
class CustomerFinancialReportExportTest {

    private lateinit var reportingService: CustomerFinancialReportingServiceImpl

    private val tenantId = "TENANT-EXP-01"
    private val projectId = "PRJ-EXP-01"
    private val customerId = "CUS-EXP-01"
    private val accountId = "CFA-EXP-01"
    ...
    @Test
    fun testExportCsvStatement() = runBlocking {
        ...
        val res = reportingService.exportFinancialReport(req)
        assertTrue(res is DomainResult.Success)
        val doc = (res as DomainResult.Success).data
        assertEquals("text/csv", doc.contentType)
        assertTrue(doc.contentString.contains("Customer Statement"))
        assertTrue(doc.contentString.contains("Export Customer"))
    }

    @Test
    fun testExportPdfDocument() = runBlocking {
        val req = CustomerFinancialReportRequest(
            tenantId = tenantId,
            projectId = projectId,
            customerId = customerId,
            reportType = CustomerFinancialReportType.FINANCIAL_SUMMARY,
            format = CustomerFinancialReportFormat.PDF
        )
        val res = reportingService.exportFinancialReport(req)
        assertTrue(res is DomainResult.Success)
        val doc = (res as DomainResult.Success).data
        assertEquals("application/pdf", doc.contentType)
        assertTrue(doc.contentString.contains("SUCHARU GRAPHICS / SUCHARU PRO ERP"))
        assertTrue(doc.contentString.contains("Total Invoiced"))
    }
}
