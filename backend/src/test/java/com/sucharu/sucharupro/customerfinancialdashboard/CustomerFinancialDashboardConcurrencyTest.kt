...
class CustomerFinancialDashboardConcurrencyTest {
        ...
        runBlocking {
            ...
            accountRepo.createAccount(
                CustomerFinancialAccount(
                    financialAccountId = "CFA-CONCUR-01",
                    tenantId = tenantId,
                    projectId = projectId,
                    customerId = customerId,
    ...
    @Test
    fun testConcurrentDashboardReads() = runBlocking {
        val deferred = (1..20).map {
            async {
                dashboardService.getCustomerFinancialDashboard(tenantId, projectId, customerId)
            }
        }
        val results = deferred.awaitAll()
        val successCount = results.count { it is DomainResult.Success }
        assertTrue("All 20 concurrent dashboard reads succeeded", successCount == 20)
    }
}
