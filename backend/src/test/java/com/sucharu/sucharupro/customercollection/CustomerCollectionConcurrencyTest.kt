...
class CustomerCollectionConcurrencyTest {
    ...
    @Test
    fun testConcurrentActionCreation() = runBlocking {
        val deferred = (1..10).map { i ->
            async {
                service.createCollectionAction(
                    tenantId = tenantId,
                    projectId = projectId,
                    customerId = customerId,
                    actionType = CollectionActionType.PHONE_FOLLOW_UP,
                    scheduledAt = System.currentTimeMillis() + (i * 1000L),
                    notes = "Concurrent action $i",
                    actorId = "agent_$i",
                    actorRole = "STAFF"
                )
            }
        ...
        assertTrue("All concurrent actions created successfully", successCount == 10)
    }
}
