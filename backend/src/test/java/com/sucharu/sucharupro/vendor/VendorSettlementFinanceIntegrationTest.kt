...
class VendorSettlementFinanceIntegrationTest {
    ...
    private lateinit var supplierPaymentDs: FakeSupplierPaymentDataSource
    private lateinit var supplierPaymentRepo: SupplierPaymentRepositoryImpl
    ...
    fun setUp() {
        runBlocking {
            ...
            val finTxDs = FakeFinancialTransactionDataSource()
            val finTxRepo = FinancialTransactionRepositoryImpl(finTxDs)
            supplierPaymentDs = FakeSupplierPaymentDataSource()
            supplierPaymentRepo = SupplierPaymentRepositoryImpl(
                dataSource = supplierPaymentDs,
                vendorPayableRepository = payableRepo,
                financialTransactionRepository = finTxRepo
            )
            ...
            analyticsRepo = VendorAnalyticsRepositoryImpl(
                ...
                paymentRepository = supplierPaymentRepo
            )
    ...
    fun testSettlementOrchestrationWithSupplierPayment() = runBlocking {
        ...
        assertTrue(processed.paymentId?.isNotBlank() == true)
    }
}
